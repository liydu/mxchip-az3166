...
VOID adu_start(NX_AZURE_IOT_HUB_CLIENT *hub_client_ptr);
...
static void adu_agent_update_notify(NX_AZURE_IOT_ADU_AGENT *adu_agent_ptr, UINT update_state, UCHAR *provider,
                                    UINT provider_length, UCHAR *name, UINT name_length, UCHAR *version,
                                    UINT version_length)
{
    ...
    if (update_state == NX_AZURE_IOT_ADU_AGENT_UPDATE_RECEIVED)
    {
        ...
        /* Received new update.  */
        printf("Received new update: Provider: %.*s; Name: %.*s, Version: %.*s\r\n", provider_length, provider,
               name_length, name, version_length, version);
...
VOID adu_start(NX_AZURE_IOT_HUB_CLIENT *hub_client_ptr)
{
    if (adu_agent_started == NX_FALSE)
    {
        ...
        /* Start ADU agent.  */
        if (nx_azure_iot_adu_agent_start(
                &adu_agent, hub_client_ptr, (const UCHAR *)SAMPLE_DEVICE_MANUFACTURER,
                sizeof(SAMPLE_DEVICE_MANUFACTURER) - 1, (const UCHAR *)SAMPLE_DEVICE_MODEL,
                sizeof(SAMPLE_DEVICE_MODEL) - 1, (const UCHAR *)SAMPLE_DEVICE_INSTALLED_CRITERIA,
                sizeof(SAMPLE_DEVICE_INSTALLED_CRITERIA) - 1, adu_agent_update_notify, nx_azure_iot_adu_agent_driver))
        {
        ...
        printf("Manufacturer: %s, Model: %s, Installed Criteria: %s.\r\n", SAMPLE_DEVICE_MANUFACTURER,
               SAMPLE_DEVICE_MODEL, SAMPLE_DEVICE_INSTALLED_CRITERIA);

#if (NX_AZURE_IOT_ADU_AGENT_PROXY_UPDATE_COUNT >= 1)
        /* Enable proxy update for leaf device.  */
        if (nx_azure_iot_adu_agent_proxy_update_add(
                &adu_agent, (const UCHAR *)SAMPLE_LEAF_DEVICE_MANUFACTURER, sizeof(SAMPLE_LEAF_DEVICE_MANUFACTURER) - 1,
        ...
        printf("Manufacturer: %s, Model: %s, Installed Criteria: %s.\r\n", SAMPLE_LEAF_DEVICE_MANUFACTURER,
               SAMPLE_LEAF_DEVICE_MODEL, SAMPLE_LEAF_DEVICE_INSTALLED_CRITERIA);
#endif /* NX_AZURE_IOT_ADU_AGENT_PROXY_UPDATE_COUNT */
