...
#ifndef DISABLE_ADU
extern VOID adu_start(NX_AZURE_IOT_HUB_CLIENT *hub_client_ptr);
#endif /* Disable ADU */
...
#ifndef DISABLE_ADU
    // Start ADU agent
    adu_start(&nx_context->iothub_client);
#endif /* Disable ADU */
...
static VOID process_command(AZURE_IOT_NX_CONTEXT *nx_context)
{
    ...
    while ((status = nx_azure_iot_hub_client_command_message_receive(
                &nx_context->iothub_client, &component_name_ptr, &component_name_length, &command_name_ptr,
                &command_name_length, &context_ptr, &context_length, &packet_ptr, NX_NO_WAIT)) == NX_AZURE_IOT_SUCCESS)
    {
        ...
        {
            nx_context->command_received_cb(nx_context, component_name_ptr, component_name_length, command_name_ptr,
                                            command_name_length, payload_ptr, payload_length, context_ptr,
                                            context_length);
        }
...
UINT azure_iot_nx_client_create(AZURE_IOT_NX_CONTEXT *nx_context, NX_IP *nx_ip, NX_PACKET_POOL *nx_pool, NX_DNS *nx_dns,
                                UINT (*unix_time_callback)(ULONG *unix_time), CHAR *iot_model_id, UINT iot_model_id_len)
{
    UINT status;
