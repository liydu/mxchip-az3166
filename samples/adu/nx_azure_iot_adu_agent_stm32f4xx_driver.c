...
void nx_azure_iot_adu_agent_driver(NX_AZURE_IOT_ADU_AGENT_DRIVER *driver_req_ptr)
{
    ...
    switch (driver_req_ptr->nx_azure_iot_adu_agent_driver_command)
    {
        ...
        case NX_AZURE_IOT_ADU_AGENT_DRIVER_PREPROCESS: {
            ...
            {
                status =
                    internal_flash_erase(FLASH_BANK_2, driver_req_ptr->nx_azure_iot_adu_agent_driver_firmware_size);
            }
            else
            {
                status =
                    internal_flash_erase(FLASH_BANK_1, driver_req_ptr->nx_azure_iot_adu_agent_driver_firmware_size);
            }
        ...
        case NX_AZURE_IOT_ADU_AGENT_DRIVER_WRITE: {
            ...
            */
            ...
            UCHAR *dest_ptr =
                (UCHAR *)(FLASH_BANK2_ADDR + driver_req_ptr->nx_azure_iot_adu_agent_driver_firmware_data_offset);
            status = internal_flash_write(dest_ptr, driver_req_ptr->nx_azure_iot_adu_agent_driver_firmware_data_ptr,
...
static int boot_bank_set(uint32_t bank)
{
    ...
    switch (bank)
    {
        case FLASH_BANK_1: OBInit.USERConfig = OB_BFB2_DISABLE; break;
        case FLASH_BANK_2: OBInit.USERConfig = OB_BFB2_ENABLE; break;
        case FLASH_BANK_BOTH:
            OBInit.USERConfig =
                ((OBInit.USERConfig & OB_BFB2_ENABLE) == OB_BFB2_ENABLE) ? OB_BFB2_DISABLE : OB_BFB2_ENABLE;
            break;
        default: rc = -1;
    }
...
static int internal_flash_erase(unsigned int bank, unsigned int size)
{
    ...
    if (size > FLASH_BANK_SIZE) return -1;
...
static int internal_flash_write(unsigned char *destination_ptr, unsigned char *source_ptr, unsigned int size)
{
    ...
    {
        status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, (uint32_t)(destination_ptr - 8),
    ...
    {
        status =
            HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, (uint32_t)(destination_ptr),
